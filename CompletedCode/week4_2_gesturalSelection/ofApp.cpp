...
void ofApp::keyReleased(int key) {
    ...
    if (key == 't' || key == 'T') {
        if (dtwActive && !regressionActive) {
            dtwTrained = dtw.train(classData);
            dtwEmpty = false;
        }
        else if (!dtwActive && regressionActive) {
            if (neuralNet2Active) {
...
void ofApp::mouseDragged(int x, int y, int button) {
    ...
    if (dtwActive && !regressionActive) {
        ...
        if (dtwRecording && !dtwTrained) {
            dtwInput.push_back(x);
            dtwInput.push_back(y);
            tempSeries.input.push_back(dtwInput);
        ...
        if (dtwRunning && dtwTrained) {
            ...
            if (outputLabel == "1") {
                neuralNet1Active = true;
                neuralNet2Active = false;
            }
            else if (outputLabel == "2") {
                neuralNet1Active = false;
                neuralNet2Active = true;
            }
            else if (outputLabel == "3") {
                neuralNet1Active = true;
                neuralNet2Active = true;
            }
        }
...
void ofApp::mouseReleased(int x, int y, int button) {
    ...
    if (dtwActive && !regressionActive && dtwRecording && !dtwTrained) {
        classData.push_back(tempSeries);
